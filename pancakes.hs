...
next_elem f n = [f++[x] | x<-[2..n], x /= (last f)] --prosthhkh 2,...,n sto telos tou f
...
batch (x:xs) = depth_stack (initial (length x)) (initial_zero (length x)) [] ( thesis (map (\x->positions x) (x:xs)) 1) (x:xs)
...
flop_b n xs r | (n == 1 && (snd (head xs)) == 1)= reverse (1:r)
            ...
            | ((max_index n xs) /= 1 && (snd (nth (max_index n xs) xs) == 0)) = flop_b (n-1) (nreve n (nreve (max_index n xs) xs)) (n:((max_index n xs):r))
            | ((max_index n xs) /= 1 && (snd (nth (max_index n xs) xs) == 1)) = flop_b (n-1) (nreve n (nreve 1 (nreve (max_index n xs) xs))) (n:(1:((max_index n xs):r)))
            | ((max_index n xs) == 1 && (snd (head xs)) == 0) = flop_b (n-1) (nreve n (nreve 1 xs)) (n:(1:r))
            | otherwise = flop_b (n-1) (nreve n xs) (n:r)
...
goal_b a ls= checkList (last (visualize ls a)) && check_burnt (last (visualize ls a))

depth_b (f:fifo) ls visited | goal f ls= f
--                        | (elem (last (visualize ls f)) visited == True) =  depth fifo ls visited
                        | otherwise = depth_b (fifo++(next_elem f (length ls))) ls ((last (visualize ls f)):visited)

burnt_bfs ls = depth_b (initial (length ls)) ls []
...
d1_b (f1:fifo1) fifo2 dep ls visited1 visited2 | length f1 > dep = d2 (f1:fifo1) fifo2 dep  ls visited1 []
                                            | (length f1 == dep) && (elem (last (visualize ls f1)) ( map (\x->fst x) visited2 )== True) = f1++(reverse (find_tuple (last (visualize ls f1)) visited2))
                                            | otherwise = d1_b (fifo1++( (next_elem f1 (length ls)))) fifo2 dep ls (((last (visualize ls f1)),f1):visited1) visited2
...
d2_b fifo1 (f2:fifo2) dep ls visited1 visited2 | length f2 > dep = d1 fifo1 (f2:fifo2) (dep+1)  ls [] visited2
                                ...
                                | otherwise = d2_b fifo1 (fifo2++( (next_elem f2 (length ls)))) dep ls visited1 (((last (visualize (sort (zero_tup ls)) f2)),f2):visited2)

burnt_bidirectional ls = d1_b (initial (length ls)) (initial (length ls)) 1 ls [(ls,[])] [((sort (zero_tup ls)),[])]
